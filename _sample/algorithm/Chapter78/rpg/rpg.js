...
function mainloop() {
    var i, n, r, x, y, w, h, col, btn;//汎用的に使う変数
    counter++;
    if(scene < 40) {//背景の表示
    ...
    if(scene==1 || scene==2) {//パーティメンバーのイラストを表示
    ...
    switch(scene) {
        ...
        if(counter == 10) {//イベントの発生？
            event = 0;
            if(flg[FLG_EVENT+1]==0 && gtime>=10*60*30) event = FLG_EVENT+1;//ゼノンが加わる
            if(flg[FLG_EVENT+2]==0 && gtime>=20*60*30) event = FLG_EVENT+2;//ビーナスが加わる
            if(flg[FLG_DOME+1]==0 && item[3]>0) event = FLG_DOME+1;//DOME 7に行けるようになる
            if(flg[FLG_DOME+2]==0 && item[5]>0) event = FLG_DOME+2;//DOME XXXに行けるようになる
            if(event > 0) {
                scene = 2;
                counter = 0;
            }
        ...
        if(counter < 40) break;
        if(counter == 40) {
            if(rnd(100)<50+20*(item[4]+item[6])) {
                setMsg("クリーチャーを発見！");
                scene = 50;
                counter = 0;
                break;
        ...
        if(scene == 51) {
            ...
            if(counter == 240) setMsg("捕獲済みのクリーチャーが逃げ出した。");
            if(counter == 360) setMsg("居住ドームに強制送還されます。");
            if(counter == 510) {
            ...
            if(counter == 30 && rnd(100) < 20) {
                setMsg("しかし敵に回り込まれた！");
                scene = 52;
            }
            if(counter == 60) stopBgm();
            if(counter == 90) {
                setMsg("探索を続けますか？");
                playBgm(1);
                scene = 40;
